...
export class AccountService {
    ...
    public async fetchAccountBalance(): Promise<Balance> {
        ...
        return {
            total: this.countAmount(transactions, TransactionType.DEBIT) - this.countAmount(transactions, TransactionType.CREDIT)
        }
    }
    ...
    private countAmount(transactions: Transaction[], type: TransactionType): number {
        ...
        return filteredTransactions.map(transaction => transaction.amount)
